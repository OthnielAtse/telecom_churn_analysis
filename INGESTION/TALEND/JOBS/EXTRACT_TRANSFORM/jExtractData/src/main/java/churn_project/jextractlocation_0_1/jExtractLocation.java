...
package churn_project.jextractlocation_0_1;
...
/**
 * Job: jExtractLocation Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class jExtractLocation implements TalendJob {
...
public String getProjectFolder(){
	...
	private final String jobName = "jExtractLocation";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				jExtractLocation.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(jExtractLocation.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
java.io.File file_tFileExist_1 = new java.io.File(context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" + "CHURN_"+ TalendDate.getDate("YYYYMMDD") +"_LOCATION.csv");
...
globalMap.put("tFileExist_1_FILENAME",context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" + "CHURN_"+ TalendDate.getDate("YYYYMMDD") +"_LOCATION.csv");
...
public static class locationStruct implements routines.system.IPersistableRow<locationStruct> {
    final static byte[] commonByteArrayLock_CHURN_PROJECT_jExtractLocation = new byte[0];
    static byte[] commonByteArray_CHURN_PROJECT_jExtractLocation = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_CHURN_PROJECT_jExtractLocation.length) {
				if(length < 1024 && commonByteArray_CHURN_PROJECT_jExtractLocation.length == 0) {
   					commonByteArray_CHURN_PROJECT_jExtractLocation = new byte[1024];
				} else {
   					commonByteArray_CHURN_PROJECT_jExtractLocation = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_CHURN_PROJECT_jExtractLocation, 0, length);
			strReturn = new String(commonByteArray_CHURN_PROJECT_jExtractLocation, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_CHURN_PROJECT_jExtractLocation, 0, length);
			strReturn = new String(commonByteArray_CHURN_PROJECT_jExtractLocation, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_CHURN_PROJECT_jExtractLocation) {
     ...
     * Compare keys
     */
    public int compareTo(locationStruct other) {
...
locationStruct location = new locationStruct();
	...
	currentComponent="tFileOutputDelimited_1";
					...
					if(execStat) {
						runStat.updateStatOnConnection(resourceMap,iterateId,0,0,"location");
					}
...
String fileName_tFileOutputDelimited_1 = "";
    fileName_tFileOutputDelimited_1 = (new java.io.File(context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" + "CHURN_"+ TalendDate.getDate("YYYYMMDD") +"_LOCATION.csv")).getAbsolutePath().replace("\\","/");
...
locationStruct location_tmp = new locationStruct();
// ###############################
...
customers_tmp.LocationID = null;
customers_tmp.City = StringHandling.TRIM(row1.City) ;
customers_tmp.ZipCode = StringHandling.TRIM(row1.ZipCode) ;
customers_tmp.Lattitude = row1.Lattitude ;
customers_tmp.Longitude = row1.Longitude ;
...
if(customers.City == null){
	...
	finder_tUniqRow_1.City = customers.City.toLowerCase();
}
if(customers.ZipCode == null){
	finder_tUniqRow_1.ZipCode = null;
}else{
	finder_tUniqRow_1.ZipCode = customers.ZipCode.toLowerCase();
}	
...
if(customers.City == null){
	...
	new_tUniqRow_1.City = customers.City.toLowerCase();
}
if(customers.ZipCode == null){
	new_tUniqRow_1.ZipCode = null;
}else{
	new_tUniqRow_1.ZipCode = customers.ZipCode.toLowerCase();
}
		
		keystUniqRow_1.add(new_tUniqRow_1);if(row2 == null){ 
	
	row2 = new row2Struct();
}row2.LocationID = customers.LocationID;			row2.City = customers.City;			row2.ZipCode = customers.ZipCode;			row2.Lattitude = customers.Lattitude;			row2.Longitude = customers.Longitude;					
...
location_tmp.LocationID = Var.var1 ;
location_tmp.City = row2.City ;
location_tmp.ZipCode = row2.ZipCode ;
location_tmp.Lattitude = row2.Lattitude ;
location_tmp.Longitude = row2.Longitude ;
 ...
 */
	...
	currentComponent="tFileOutputDelimited_1";
		...
		resourceMap.put("finish_tFileOutputDelimited_1", true);
				...
				if(execStat){
			  		runStat.updateStat(resourceMap,iterateId,2,0,"location");
			  	}
 ...
 */
	...
	System.err.println("Le fichier " +"CHURN_"+ TalendDate.getDate("YYYYMMDD") +" _LOCATION.csv" +"  existe déjà dans le repertoire du jour " + context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" );
	
	globalMap.put("tDie_1_DIE_MESSAGE", "Le fichier " +"CHURN_"+ TalendDate.getDate("YYYYMMDD") +" _LOCATION.csv" +"  existe déjà dans le repertoire du jour " + context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" );
	globalMap.put("tDie_1_DIE_MESSAGES", "Le fichier " +"CHURN_"+ TalendDate.getDate("YYYYMMDD") +" _LOCATION.csv" +"  existe déjà dans le repertoire du jour " + context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" );
 ...
 */
    ...
    public static void main(String[] args){
        final jExtractLocation jExtractLocationClass = new jExtractLocation();

        int exitCode = jExtractLocationClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = jExtractLocation.class.getClassLoader().getResourceAsStream("churn_project/jextractlocation_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = jExtractLocation.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : jExtractLocation");
        }
...
/************************************************************************************************
 *     163139 characters generated by Talend Open Studio for Data Integration 
