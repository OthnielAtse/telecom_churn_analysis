...
package churn_project.jextractcontract_0_1;
...
/**
 * Job: jExtractContract Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class jExtractContract implements TalendJob {
...
public String getProjectFolder(){
	...
	private final String jobName = "jExtractContract";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				jExtractContract.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(jExtractContract.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
java.io.File file_tFileExist_1 = new java.io.File(context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" + "CHURN_"+ TalendDate.getDate("YYYYMMDD") +"_CONTRACT.csv");
...
globalMap.put("tFileExist_1_FILENAME",context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" + "CHURN_"+ TalendDate.getDate("YYYYMMDD") +"_CONTRACT.csv");
...
public static class uniqueChurnStruct implements routines.system.IPersistableRow<uniqueChurnStruct> {
    final static byte[] commonByteArrayLock_CHURN_PROJECT_jExtractContract = new byte[0];
    static byte[] commonByteArray_CHURN_PROJECT_jExtractContract = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_CHURN_PROJECT_jExtractContract.length) {
				if(length < 1024 && commonByteArray_CHURN_PROJECT_jExtractContract.length == 0) {
   					commonByteArray_CHURN_PROJECT_jExtractContract = new byte[1024];
				} else {
   					commonByteArray_CHURN_PROJECT_jExtractContract = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_CHURN_PROJECT_jExtractContract, 0, length);
			strReturn = new String(commonByteArray_CHURN_PROJECT_jExtractContract, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_CHURN_PROJECT_jExtractContract, 0, length);
			strReturn = new String(commonByteArray_CHURN_PROJECT_jExtractContract, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_CHURN_PROJECT_jExtractContract) {
...
String fileName_tFileOutputDelimited_1 = "";
    fileName_tFileOutputDelimited_1 = (new java.io.File(context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" + "CHURN_"+ TalendDate.getDate("YYYYMMDD") +"_CONTRACT.csv")).getAbsolutePath().replace("\\","/");
...
customers_tmp.ContractId = null;
customers_tmp.PaperlessBilling = StringHandling.TRIM(row1.PaperlessBilling) ;
customers_tmp.Contract = StringHandling.TRIM(row1.Contract) ;
...
row2 = null;			
if(customers.PaperlessBilling == null){
	finder_tUniqRow_1.PaperlessBilling = null;
}else{
	finder_tUniqRow_1.PaperlessBilling = customers.PaperlessBilling.toLowerCase();
}
if(customers.Contract == null){
	finder_tUniqRow_1.Contract = null;
}else{
	finder_tUniqRow_1.Contract = customers.Contract.toLowerCase();
}	
...
if(customers.PaperlessBilling == null){
	new_tUniqRow_1.PaperlessBilling = null;
}else{
	new_tUniqRow_1.PaperlessBilling = customers.PaperlessBilling.toLowerCase();
}
if(customers.Contract == null){
	new_tUniqRow_1.Contract = null;
}else{
	new_tUniqRow_1.Contract = customers.Contract.toLowerCase();
}
		
		keystUniqRow_1.add(new_tUniqRow_1);if(row2 == null){ 
	
	row2 = new row2Struct();
}row2.ContractId = customers.ContractId;			row2.PaperlessBilling = customers.PaperlessBilling;			row2.Contract = customers.Contract;					
...
uniqueChurn_tmp.ContractId = Var.id ;
uniqueChurn_tmp.PaperlessBilling = row2.PaperlessBilling ;
uniqueChurn_tmp.Contract = row2.Contract ;
...
if(uniqueChurn != null) { 
	...
	currentComponent="tFileOutputDelimited_1";
                    ...
                    StringBuilder sb_tFileOutputDelimited_1 = new StringBuilder();
                            if(uniqueChurn.ContractId != null) {
                        sb_tFileOutputDelimited_1.append(
                            uniqueChurn.ContractId
                        );
                            }
                            sb_tFileOutputDelimited_1.append(OUT_DELIM_tFileOutputDelimited_1);
                            if(uniqueChurn.PaperlessBilling != null) {
                        sb_tFileOutputDelimited_1.append(
                            uniqueChurn.PaperlessBilling
                        );
                            }
                            sb_tFileOutputDelimited_1.append(OUT_DELIM_tFileOutputDelimited_1);
                            if(uniqueChurn.Contract != null) {
                        sb_tFileOutputDelimited_1.append(
                            uniqueChurn.Contract
                        );
                            }
 ...
 */
	...
	System.err.println("Le fichier " +"CHURN_"+ TalendDate.getDate("YYYYMMDD") +" _CONTRACT.csv" +"  existe déjà dans le repertoire du jour "+ context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" );
	
	globalMap.put("tDie_1_DIE_MESSAGE", "Le fichier " +"CHURN_"+ TalendDate.getDate("YYYYMMDD") +" _CONTRACT.csv" +"  existe déjà dans le repertoire du jour "+ context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" );
	globalMap.put("tDie_1_DIE_MESSAGES", "Le fichier " +"CHURN_"+ TalendDate.getDate("YYYYMMDD") +" _CONTRACT.csv" +"  existe déjà dans le repertoire du jour "+ context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" );
 ...
 */
    ...
    public static void main(String[] args){
        final jExtractContract jExtractContractClass = new jExtractContract();

        int exitCode = jExtractContractClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = jExtractContract.class.getClassLoader().getResourceAsStream("churn_project/jextractcontract_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = jExtractContract.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : jExtractContract");
        }
...
/************************************************************************************************
 *     156165 characters generated by Talend Open Studio for Data Integration 
