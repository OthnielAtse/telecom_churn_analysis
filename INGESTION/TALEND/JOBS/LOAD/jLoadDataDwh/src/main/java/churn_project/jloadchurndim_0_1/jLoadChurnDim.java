...
package churn_project.jloadchurndim_0_1;
...
/**
 * Job: jLoadChurnDim Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class jLoadChurnDim implements TalendJob {
...
public String getProjectFolder(){
	...
	private final String jobName = "jLoadChurnDim";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				jLoadChurnDim.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(jLoadChurnDim.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
globalMap.put("nomFichier", "CHURN_"+ TalendDate.getDate("YYYYMMDD") +"_CHURN.csv");
globalMap.put("nomFlux", "CHURN");
...
System.out.println((Integer)globalMap.get("tDBOutput_1_NB_LINE_REJECTED"));
...
public static class insertChurnStruct implements routines.system.IPersistableRow<insertChurnStruct> {
    final static byte[] commonByteArrayLock_CHURN_PROJECT_jLoadChurnDim = new byte[0];
    static byte[] commonByteArray_CHURN_PROJECT_jLoadChurnDim = new byte[0];
	...
	public boolean equals(Object obj) {
		...
		final insertChurnStruct other = (insertChurnStruct) obj;
	...
	public void copyDataTo(insertChurnStruct other) {
	...
	public void copyKeysDataTo(insertChurnStruct other) {
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_CHURN_PROJECT_jLoadChurnDim.length) {
				if(length < 1024 && commonByteArray_CHURN_PROJECT_jLoadChurnDim.length == 0) {
   					commonByteArray_CHURN_PROJECT_jLoadChurnDim = new byte[1024];
				} else {
   					commonByteArray_CHURN_PROJECT_jLoadChurnDim = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_CHURN_PROJECT_jLoadChurnDim, 0, length);
			strReturn = new String(commonByteArray_CHURN_PROJECT_jLoadChurnDim, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_CHURN_PROJECT_jLoadChurnDim, 0, length);
			strReturn = new String(commonByteArray_CHURN_PROJECT_jLoadChurnDim, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_CHURN_PROJECT_jLoadChurnDim) {
        	...
        	try {
        		...
        		int length = 0;
					...
					this.CHURN_CATEGORY = readString(dis);
					
					this.CHURN_REASON = readString(dis);
    ...
    public void writeData(ObjectOutputStream dos) {
        try {
					...
					// String
						...
						writeString(this.CHURN_CATEGORY,dos);
					
					// String
				
						writeString(this.CHURN_REASON,dos);
     ...
     * Compare keys
     */
    public int compareTo(insertChurnStruct other) {
...
insertChurnStruct insertChurn = new insertChurnStruct();
	...
	currentComponent="tDBOutput_1";
					...
					if(execStat) {
						runStat.updateStatOnConnection(resourceMap,iterateId,0,0,"insertChurn");
					}
...
if(dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
	tableName_tDBOutput_1 = ("DWH_DIM_CHURN");
} else {
	tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "\".\"" + ("DWH_DIM_CHURN");
}
...
int count_tDBOutput_1=0;
	    java.sql.PreparedStatement pstmt_tDBOutput_1 = conn_tDBOutput_1.prepareStatement("SELECT COUNT(1) FROM \"" + tableName_tDBOutput_1 + "\" WHERE \"CHURN_ID\" = ?");
	    resourceMap.put("pstmt_tDBOutput_1", pstmt_tDBOutput_1);
	    String insert_tDBOutput_1 = "INSERT INTO \"" + tableName_tDBOutput_1 + "\" (\"CHURN_ID\",\"CUSTOMER_STATUS\",\"CHURN_CATEGORY\",\"CHURN_REASON\",\"LB_NOM_FICHIER\",\"DT_INSERTION\",\"LB_JOB_NAME\") VALUES (?,?,?,?,?,?,?)";
	    java.sql.PreparedStatement pstmtInsert_tDBOutput_1 = conn_tDBOutput_1.prepareStatement(insert_tDBOutput_1);
	    resourceMap.put("pstmtInsert_tDBOutput_1", pstmtInsert_tDBOutput_1);
	    String update_tDBOutput_1 = "UPDATE \"" + tableName_tDBOutput_1 + "\" SET \"CUSTOMER_STATUS\" = ?,\"CHURN_CATEGORY\" = ?,\"CHURN_REASON\" = ?,\"LB_NOM_FICHIER\" = ?,\"DT_INSERTION\" = ?,\"LB_JOB_NAME\" = ? WHERE \"CHURN_ID\" = ?";
...
insertChurnStruct insertChurn_tmp = new insertChurnStruct();
// ###############################
...
globalMap.put("tFileInputDelimited_1_ERROR_MESSAGE",ex_tFileInputDelimited_1.getMessage());
					...
					columnIndexWithD_tFileInputDelimited_1 = 1;
							...
							row1.CustomerStatus = fid_tFileInputDelimited_1.get(columnIndexWithD_tFileInputDelimited_1);
						
				
					columnIndexWithD_tFileInputDelimited_1 = 2;
					
							row1.ChurnCategory = fid_tFileInputDelimited_1.get(columnIndexWithD_tFileInputDelimited_1);
						
				
					columnIndexWithD_tFileInputDelimited_1 = 3;
					
							row1.ChurnReason = fid_tFileInputDelimited_1.get(columnIndexWithD_tFileInputDelimited_1);
								...
								hasCasePrimitiveKeyWithNull_tMap_1 = false;
								  ...
								  if(hasCasePrimitiveKeyWithNull_tMap_1 || !tHash_Lookup_row2.hasNext()) { // G_TM_M_090
				           		...
				           		if(tHash_Lookup_row2 != null && tHash_Lookup_row2.getCount(row2HashKey) > 1) { // G 071
									...
									//System.out.println("WARNING: UNIQUE MATCH is configured for the lookup 'row2' and it contains more one result from keys :  row2.CHURN_ID = '" + row2HashKey.CHURN_ID + "'");
								} // G 071
...
// # Output reject table : 'insertChurn'
// # Filter conditions 
if( rejectedInnerJoin_tMap_1 ) {
insertChurn_tmp.CHURN_ID = row1.ChurnId ;
insertChurn_tmp.CUSTOMER_STATUS = row1.CustomerStatus ;
insertChurn_tmp.CHURN_CATEGORY = row1.ChurnCategory ;
insertChurn_tmp.CHURN_REASON = row1.ChurnReason ;
insertChurn_tmp.LB_NOM_FICHIER = ((String)globalMap.get("nomFichier")) ;
insertChurn_tmp.DT_INSERTION = TalendDate.getCurrentDate() ;
insertChurn_tmp.LB_JOB_NAME = jobName;
insertChurn = insertChurn_tmp;
...
if(insertChurn != null) { 
	...
	currentComponent="tDBOutput_1";
        ...
        whetherReject_tDBOutput_1 = false;
                    pstmt_tDBOutput_1.setInt(1, insertChurn.CHURN_ID);
            ...
            if(checkCount_tDBOutput_1 > 0) {
                        if(insertChurn.CUSTOMER_STATUS == null) {
pstmtUpdate_tDBOutput_1.setNull(1, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(1, insertChurn.CUSTOMER_STATUS);
}

                        if(insertChurn.CHURN_CATEGORY == null) {
pstmtUpdate_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(2, insertChurn.CHURN_CATEGORY);
}

                        if(insertChurn.CHURN_REASON == null) {
pstmtUpdate_tDBOutput_1.setNull(3, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(3, insertChurn.CHURN_REASON);
}

                        if(insertChurn.LB_NOM_FICHIER == null) {
pstmtUpdate_tDBOutput_1.setNull(4, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(4, insertChurn.LB_NOM_FICHIER);
}

                        if(insertChurn.DT_INSERTION != null) {
pstmtUpdate_tDBOutput_1.setTimestamp(5, new java.sql.Timestamp(insertChurn.DT_INSERTION.getTime()));
} else {
pstmtUpdate_tDBOutput_1.setNull(5, java.sql.Types.TIMESTAMP);
}

                        if(insertChurn.LB_JOB_NAME == null) {
pstmtUpdate_tDBOutput_1.setNull(6, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(6, insertChurn.LB_JOB_NAME);
}

                        pstmtUpdate_tDBOutput_1.setInt(7 + count_tDBOutput_1, insertChurn.CHURN_ID);
...
globalMap.put("tDBOutput_1_ERROR_MESSAGE",e.getMessage());
                    ...
                    whetherReject_tDBOutput_1 = true;
                        ...
                        pstmtInsert_tDBOutput_1.setInt(1, insertChurn.CHURN_ID);

                        if(insertChurn.CUSTOMER_STATUS == null) {
pstmtInsert_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_1.setString(2, insertChurn.CUSTOMER_STATUS);
}

                        if(insertChurn.CHURN_CATEGORY == null) {
pstmtInsert_tDBOutput_1.setNull(3, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_1.setString(3, insertChurn.CHURN_CATEGORY);
}

                        if(insertChurn.CHURN_REASON == null) {
pstmtInsert_tDBOutput_1.setNull(4, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_1.setString(4, insertChurn.CHURN_REASON);
}

                        if(insertChurn.LB_NOM_FICHIER == null) {
pstmtInsert_tDBOutput_1.setNull(5, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_1.setString(5, insertChurn.LB_NOM_FICHIER);
}

                        if(insertChurn.DT_INSERTION != null) {
pstmtInsert_tDBOutput_1.setTimestamp(6, new java.sql.Timestamp(insertChurn.DT_INSERTION.getTime()));
} else {
...
} else {pstmtInsert_tDBOutput_1.setString(7, insertChurn.LB_JOB_NAME);
}
 ...
 */
	...
	nb_line_rejected_tDBOutput_1=nb_line_rejected_tDBOutput_1 + rejectedCount_tDBOutput_1;
        ...
        globalMap.put("tDBOutput_1_NB_LINE_REJECTED", nb_line_rejected_tDBOutput_1);
				...
				if(execStat){
			  		runStat.updateStat(resourceMap,iterateId,2,0,"insertChurn");
			  	}
 ...
 final boolean execStat = this.execStat;
	...
	currentComponent="tAdvancedHash_row2";
		...
		int tos_count_tAdvancedHash_row2 = 0;
			   		...
			   		// linked node: tMap_1 - inputs:(row1,row2) outputs:(insertChurn)
 ...
 */
	...
	final String decryptedPassword_tDBInput_1 = context.password; 
            ...
            row2.CHURN_ID = rs_tDBInput_1.getInt(1);
            if(rs_tDBInput_1.wasNull()){
                    throw new RuntimeException("Null value in non-Nullable column");
            }
		                    }
							if(colQtyInRs_tDBInput_1 < 2) {
								row2.CUSTOMER_STATUS = null;
							} else {
	                         		
        	row2.CUSTOMER_STATUS = routines.system.JDBCUtil.getString(rs_tDBInput_1, 2, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 3) {
								row2.CHURN_CATEGORY = null;
							} else {
	                         		
        	row2.CHURN_CATEGORY = routines.system.JDBCUtil.getString(rs_tDBInput_1, 3, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 4) {
								row2.CHURN_REASON = null;
							} else {
	                         		
        	row2.CHURN_REASON = routines.system.JDBCUtil.getString(rs_tDBInput_1, 4, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 5) {
								row2.LB_NOM_FICHIER = null;
							} else {
	                         		
        	row2.LB_NOM_FICHIER = routines.system.JDBCUtil.getString(rs_tDBInput_1, 5, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 6) {
								row2.DT_INSERTION = null;
							} else {
										
			row2.DT_INSERTION = routines.system.JDBCUtil.getDate(rs_tDBInput_1, 6);
		                    }
							if(colQtyInRs_tDBInput_1 < 7) {
								row2.LB_JOB_NAME = null;
							} else {
	                         		
        	row2.LB_JOB_NAME = routines.system.JDBCUtil.getString(rs_tDBInput_1, 7, false);
		                    }
 ...
 */
	...
	currentComponent="tAdvancedHash_row2";
				...
				row2_HashRow.CUSTOMER_STATUS = row2.CUSTOMER_STATUS;
				
				row2_HashRow.CHURN_CATEGORY = row2.CHURN_CATEGORY;
				
				row2_HashRow.CHURN_REASON = row2.CHURN_REASON;
 ...
 */
    ...
    public static void main(String[] args){
        final jLoadChurnDim jLoadChurnDimClass = new jLoadChurnDim();

        int exitCode = jLoadChurnDimClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = jLoadChurnDim.class.getClassLoader().getResourceAsStream("churn_project/jloadchurndim_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = jLoadChurnDim.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : jLoadChurnDim");
        }
...
/************************************************************************************************
 *     149917 characters generated by Talend Open Studio for Data Integration 
 *     on the 30 décembre 2025 à 22:41:58 EET
 ************************************************************************************************/