...
package churn_project.jloadcustomerdim_0_1;
...
/**
 * Job: jLoadCustomerDim Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class jLoadCustomerDim implements TalendJob {
...
public String getProjectFolder(){
	...
	private final String jobName = "jLoadCustomerDim";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				jLoadCustomerDim.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(jLoadCustomerDim.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
globalMap.put("nomFichier", "CHURN_"+ TalendDate.getDate("YYYYMMDD") +"_CUSTOMER.csv");
globalMap.put("nomFlux", "CUSTOMER");
...
public static class insertCustomerStruct implements routines.system.IPersistableRow<insertCustomerStruct> {
    final static byte[] commonByteArrayLock_CHURN_PROJECT_jLoadCustomerDim = new byte[0];
    static byte[] commonByteArray_CHURN_PROJECT_jLoadCustomerDim = new byte[0];
	...
	public boolean equals(Object obj) {
		...
		final insertCustomerStruct other = (insertCustomerStruct) obj;
	...
	public void copyDataTo(insertCustomerStruct other) {
	...
	public void copyKeysDataTo(insertCustomerStruct other) {
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_CHURN_PROJECT_jLoadCustomerDim.length) {
				if(length < 1024 && commonByteArray_CHURN_PROJECT_jLoadCustomerDim.length == 0) {
   					commonByteArray_CHURN_PROJECT_jLoadCustomerDim = new byte[1024];
				} else {
   					commonByteArray_CHURN_PROJECT_jLoadCustomerDim = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_CHURN_PROJECT_jLoadCustomerDim, 0, length);
			strReturn = new String(commonByteArray_CHURN_PROJECT_jLoadCustomerDim, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_CHURN_PROJECT_jLoadCustomerDim, 0, length);
			strReturn = new String(commonByteArray_CHURN_PROJECT_jLoadCustomerDim, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_CHURN_PROJECT_jLoadCustomerDim) {
     ...
     * Compare keys
     */
    public int compareTo(insertCustomerStruct other) {
...
insertCustomerStruct insertCustomer = new insertCustomerStruct();
InsertRejectStruct InsertReject = new InsertRejectStruct();
	...
	currentComponent="tDBOutput_2";
					...
					if(execStat) {
						runStat.updateStatOnConnection(resourceMap,iterateId,0,0,"insertCustomer");
					}
...
if(dbschema_tDBOutput_2 == null || dbschema_tDBOutput_2.trim().length() == 0) {
	tableName_tDBOutput_2 = ("DWH_DIM_CUSTOMER");
} else {
	tableName_tDBOutput_2 = dbschema_tDBOutput_2 + "\".\"" + ("DWH_DIM_CUSTOMER");
}
...
int count_tDBOutput_2=0;
	    java.sql.PreparedStatement pstmt_tDBOutput_2 = conn_tDBOutput_2.prepareStatement("SELECT COUNT(1) FROM \"" + tableName_tDBOutput_2 + "\" WHERE \"CUSTOMER_ID\" = ?");
	    resourceMap.put("pstmt_tDBOutput_2", pstmt_tDBOutput_2);
	    String insert_tDBOutput_2 = "INSERT INTO \"" + tableName_tDBOutput_2 + "\" (\"CUSTOMER_ID\",\"LOCATION_ID\",\"GENDER\",\"AGE\",\"MARRIED\",\"NUMBER_DEPENDENTS\",\"NUMBER_REFERRALS\",\"LB_NOM_FICHIER\",\"DT_INSERTION\",\"LB_JOB_NAME\") VALUES (?,?,?,?,?,?,?,?,?,?)";
	    ...
	    String update_tDBOutput_2 = "UPDATE \"" + tableName_tDBOutput_2 + "\" SET \"LOCATION_ID\" = ?,\"GENDER\" = ?,\"AGE\" = ?,\"MARRIED\" = ?,\"NUMBER_DEPENDENTS\" = ?,\"NUMBER_REFERRALS\" = ?,\"LB_NOM_FICHIER\" = ?,\"DT_INSERTION\" = ?,\"LB_JOB_NAME\" = ? WHERE \"CUSTOMER_ID\" = ?";
...
insertCustomerStruct insertCustomer_tmp = new insertCustomerStruct();
					...
					columnIndexWithD_tFileInputDelimited_1 = 1;
						...
						if(temp.length() > 0) {
								...
								try {
    								...
    								row1.LocationID = ParserUtils.parseTo_Integer(temp);
...
globalMap.put("tFileInputDelimited_1_ERROR_MESSAGE",ex_tFileInputDelimited_1.getMessage());
									rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format("Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
										"LocationID", "row1", temp, ex_tFileInputDelimited_1), ex_tFileInputDelimited_1));
								}
    							
						} else {						
							
								
									row1.LocationID = null;
...
globalMap.put("tFileInputDelimited_1_ERROR_MESSAGE",ex_tFileInputDelimited_1.getMessage());
					...
					columnIndexWithD_tFileInputDelimited_1 = 4;
							...
							row1.Married = fid_tFileInputDelimited_1.get(columnIndexWithD_tFileInputDelimited_1);
					...
					columnIndexWithD_tFileInputDelimited_1 = 5;
						...
						if(temp.length() > 0) {
								...
								try {
    								...
    								row1.NumberOfDependents = ParserUtils.parseTo_Integer(temp);
    							
    							} catch(java.lang.Exception ex_tFileInputDelimited_1) {
globalMap.put("tFileInputDelimited_1_ERROR_MESSAGE",ex_tFileInputDelimited_1.getMessage());
									rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format("Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
										"NumberOfDependents", "row1", temp, ex_tFileInputDelimited_1), ex_tFileInputDelimited_1));
								}
					...
					columnIndexWithD_tFileInputDelimited_1 = 6;
						...
						if(temp.length() > 0) {
								...
								try {
    								...
    								row1.NumberOfReferrals = ParserUtils.parseTo_Integer(temp);
    							
    							} catch(java.lang.Exception ex_tFileInputDelimited_1) {
globalMap.put("tFileInputDelimited_1_ERROR_MESSAGE",ex_tFileInputDelimited_1.getMessage());
									rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format("Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
										"NumberOfReferrals", "row1", temp, ex_tFileInputDelimited_1), ex_tFileInputDelimited_1));
								}
...
insertServices_tmp.LOCATION_ID = row1.LocationID ;
insertServices_tmp.GENDER = row1.Gender ;
insertServices_tmp.AGE = row1.Age ;
insertServices_tmp.MARRIED = row1.Married ;
insertServices_tmp.NUMBER_DEPENDENTS = row1.NumberOfDependents ;
insertServices_tmp.NUMBER_REFERRALS = row1.NumberOfReferrals ;
 					    ...
 					    boolean forceLooprow3 = false;
 							...
 							row3Struct row3ObjectFromLookup = null;
								...
								hasCasePrimitiveKeyWithNull_tMap_2 = false;
	                        		    	...
	                        		    	Object exprKeyValue_row3__LOCATION_ID = insertServices.LOCATION_ID ;
	                        		    	if(exprKeyValue_row3__LOCATION_ID == null) {
	                        		    		hasCasePrimitiveKeyWithNull_tMap_2 = true;
	                        		    	} else {
                        		    			row3HashKey.LOCATION_ID = (int)(Integer) exprKeyValue_row3__LOCATION_ID;
                        		    		}
				           		...
				           		if(tHash_Lookup_row3 != null && tHash_Lookup_row3.getCount(row3HashKey) > 1) { // G 071
									...
									//System.out.println("WARNING: UNIQUE MATCH is configured for the lookup 'row3' and it contains more one result from keys :  row3.LOCATION_ID = '" + row3HashKey.LOCATION_ID + "'");
								} // G 071
...
// # Output table : 'insertCustomer'
insertCustomer_tmp.CUSTOMER_ID = insertServices.CUSTOMER_ID ;
insertCustomer_tmp.LOCATION_ID = insertServices.LOCATION_ID ;
insertCustomer_tmp.GENDER = insertServices.GENDER ;
insertCustomer_tmp.AGE = insertServices.AGE ;
insertCustomer_tmp.MARRIED = insertServices.MARRIED ;
insertCustomer_tmp.NUMBER_DEPENDENTS = insertServices.NUMBER_DEPENDENTS ;
insertCustomer_tmp.NUMBER_REFERRALS = insertServices.NUMBER_REFERRALS ;
insertCustomer_tmp.LB_NOM_FICHIER = insertServices.LB_NOM_FICHIER ;
insertCustomer_tmp.DT_INSERTION = insertServices.DT_INSERTION ;
insertCustomer_tmp.LB_JOB_NAME = insertServices.LB_JOB_NAME ;
insertCustomer = insertCustomer_tmp;
// ###### START REJECTS ##### 
// ###############################
 ...
 */
// Start of branch "insertCustomer"
if(insertCustomer != null) { 
	...
	currentComponent="tDBOutput_2";
        ...
        whetherReject_tDBOutput_2 = false;
                    if(insertCustomer.CUSTOMER_ID == null) {
pstmt_tDBOutput_2.setNull(1, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_2.setString(1, insertCustomer.CUSTOMER_ID);
}
            ...
            if(checkCount_tDBOutput_2 > 0) {
                        pstmtUpdate_tDBOutput_2.setInt(1, insertCustomer.LOCATION_ID);

                        if(insertCustomer.GENDER == null) {
pstmtUpdate_tDBOutput_2.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_2.setString(2, insertCustomer.GENDER);
}

                        pstmtUpdate_tDBOutput_2.setInt(3, insertCustomer.AGE);

                        if(insertCustomer.MARRIED == null) {
pstmtUpdate_tDBOutput_2.setNull(4, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_2.setString(4, insertCustomer.MARRIED);
}

                        pstmtUpdate_tDBOutput_2.setInt(5, insertCustomer.NUMBER_DEPENDENTS);

                        pstmtUpdate_tDBOutput_2.setInt(6, insertCustomer.NUMBER_REFERRALS);

                        if(insertCustomer.LB_NOM_FICHIER == null) {
pstmtUpdate_tDBOutput_2.setNull(7, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_2.setString(7, insertCustomer.LB_NOM_FICHIER);
}

                        if(insertCustomer.DT_INSERTION != null) {
pstmtUpdate_tDBOutput_2.setTimestamp(8, new java.sql.Timestamp(insertCustomer.DT_INSERTION.getTime()));
} else {
pstmtUpdate_tDBOutput_2.setNull(8, java.sql.Types.TIMESTAMP);
}

                        if(insertCustomer.LB_JOB_NAME == null) {
pstmtUpdate_tDBOutput_2.setNull(9, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_2.setString(9, insertCustomer.LB_JOB_NAME);
}

                        if(insertCustomer.CUSTOMER_ID == null) {
pstmtUpdate_tDBOutput_2.setNull(10 + count_tDBOutput_2, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_2.setString(10 + count_tDBOutput_2, insertCustomer.CUSTOMER_ID);
}
...
} else {pstmtInsert_tDBOutput_2.setString(1, insertCustomer.CUSTOMER_ID);
}

                        pstmtInsert_tDBOutput_2.setInt(2, insertCustomer.LOCATION_ID);

                        if(insertCustomer.GENDER == null) {
pstmtInsert_tDBOutput_2.setNull(3, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_2.setString(3, insertCustomer.GENDER);
}

                        pstmtInsert_tDBOutput_2.setInt(4, insertCustomer.AGE);

                        if(insertCustomer.MARRIED == null) {
pstmtInsert_tDBOutput_2.setNull(5, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_2.setString(5, insertCustomer.MARRIED);
}

                        pstmtInsert_tDBOutput_2.setInt(6, insertCustomer.NUMBER_DEPENDENTS);

                        pstmtInsert_tDBOutput_2.setInt(7, insertCustomer.NUMBER_REFERRALS);

                        if(insertCustomer.LB_NOM_FICHIER == null) {
pstmtInsert_tDBOutput_2.setNull(8, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_2.setString(8, insertCustomer.LB_NOM_FICHIER);
}

                        if(insertCustomer.DT_INSERTION != null) {
pstmtInsert_tDBOutput_2.setTimestamp(9, new java.sql.Timestamp(insertCustomer.DT_INSERTION.getTime()));
} else {
...
} else {pstmtInsert_tDBOutput_2.setString(10, insertCustomer.LB_JOB_NAME);
}
...
} else {pstmt_tDBOutput_1.setString(5, InsertReject.LB_MESSAGE_REJET);
}

                    if(InsertReject.LB_NOM_FLUX == null) {
pstmt_tDBOutput_1.setNull(6, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(6, InsertReject.LB_NOM_FLUX);
}

                    if(InsertReject.DT_REJET != null) {
pstmt_tDBOutput_1.setTimestamp(7, new java.sql.Timestamp(InsertReject.DT_REJET.getTime()));
} else {
...
} else {pstmt_tDBOutput_1.setString(8, InsertReject.LB_JOB_NAME);
}
 ...
 */
	...
	nb_line_rejected_tDBOutput_2=nb_line_rejected_tDBOutput_2 + rejectedCount_tDBOutput_2;
        ...
        globalMap.put("tDBOutput_2_NB_LINE_REJECTED", nb_line_rejected_tDBOutput_2);
				...
				if(execStat){
			  		runStat.updateStat(resourceMap,iterateId,2,0,"insertCustomer");
			  	}
 ...
 */
	...
	final String decryptedPassword_tDBInput_1 = context.password; 
            ...
            row2.LOCATION_ID = rs_tDBInput_1.getInt(2);
        	...
        	row2.GENDER = routines.system.JDBCUtil.getString(rs_tDBInput_1, 3, false);
		                    }
        	...
        	row2.MARRIED = routines.system.JDBCUtil.getString(rs_tDBInput_1, 5, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 6) {
								row2.NUMBER_DEPENDENTS = 0;
							} else {
		                          
            row2.NUMBER_DEPENDENTS = rs_tDBInput_1.getInt(6);
            if(rs_tDBInput_1.wasNull()){
                    throw new RuntimeException("Null value in non-Nullable column");
            }
		                    }
							if(colQtyInRs_tDBInput_1 < 7) {
								row2.NUMBER_REFERRALS = 0;
							} else {
		                          
            row2.NUMBER_REFERRALS = rs_tDBInput_1.getInt(7);
            if(rs_tDBInput_1.wasNull()){
                    throw new RuntimeException("Null value in non-Nullable column");
            }
		                    }
        	...
        	row2.LB_NOM_FICHIER = routines.system.JDBCUtil.getString(rs_tDBInput_1, 8, false);
		                    }
							if(colQtyInRs_tDBInput_1 < 9) {
								row2.DT_INSERTION = null;
							} else {
										
			row2.DT_INSERTION = routines.system.JDBCUtil.getDate(rs_tDBInput_1, 9);
		                    }
							if(colQtyInRs_tDBInput_1 < 10) {
								row2.LB_JOB_NAME = null;
							} else {
	                         		
        	row2.LB_JOB_NAME = routines.system.JDBCUtil.getString(rs_tDBInput_1, 10, false);
		                    }
 ...
 */
	...
	currentComponent="tAdvancedHash_row2";
				...
				row2_HashRow.MARRIED = row2.MARRIED;
				
				row2_HashRow.NUMBER_DEPENDENTS = row2.NUMBER_DEPENDENTS;
				
				row2_HashRow.NUMBER_REFERRALS = row2.NUMBER_REFERRALS;
...
public static class row3Struct implements routines.system.IPersistableComparableLookupRow<row3Struct> {
    ...
    public void readValuesData(DataInputStream dis, org.jboss.marshalling.Unmarshaller objectIn) {
        try {
			int length = 0;
						...
						this.ZIP_CODE = readString(dis,objectIn);
    ...
    public void writeValuesData(DataOutputStream dos, org.jboss.marshalling.Marshaller objectOut){
                try {
						...
						writeString(this.ZIP_CODE, dos, objectOut);
 ...
 final boolean execStat = this.execStat;
	...
	currentComponent="tAdvancedHash_row3";
		...
		int tos_count_tAdvancedHash_row3 = 0;
			   		...
			   		// linked node: tMap_2 - inputs:(insertServices,row3) outputs:(insertCustomer,InsertReject)
 ...
 */
	...
	final String decryptedPassword_tDBInput_2 = context.password; 
		    ...
		    String dbquery_tDBInput_2 = "SELECT \n  \""+context.schemaDwh+"\".\"DWH_DIM_LOCATION\".\"LOCATION_ID\", \n  \""+context.schemaDwh+"\".\"DWH_DIM_LOCATION\".\"CITY\", \n  \""+context.schemaDwh+"\".\"DWH_DIM_LOCATION\".\"ZIP_CODE\", \n  \""+context.schemaDwh+"\".\"DWH_DIM_LOCATION\".\"LATITUDE\", \n  \""+context.schemaDwh+"\".\"DWH_DIM_LOCATION\".\"LONGITUDE\", \n  \""+context.schemaDwh+"\".\"DWH_DIM_LOCATION\".\"LB_NOM_FICHIER\", \n  \""+context.schemaDwh+"\".\"DWH_DIM_LOCATION\".\"DT_INSERTION\", \n  \""+context.schemaDwh+"\".\"DWH_DIM_LOCATION\".\"LB_JOB_NAME\"\nFROM \""+context.schemaDwh+"\".\"DWH_DIM_LOCATION\"";
            ...
            row3.LOCATION_ID = rs_tDBInput_2.getInt(1);
        	...
        	row3.CITY = routines.system.JDBCUtil.getString(rs_tDBInput_2, 2, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 3) {
								row3.ZIP_CODE = null;
							} else {
	                         		
        	row3.ZIP_CODE = routines.system.JDBCUtil.getString(rs_tDBInput_2, 3, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 4) {
								row3.LATITUDE = 0;
							} else {
	                         		
            row3.LATITUDE = rs_tDBInput_2.getDouble(4);
            if(rs_tDBInput_2.wasNull()){
                    throw new RuntimeException("Null value in non-Nullable column");
            }
		                    }
            ...
            row3.LONGITUDE = rs_tDBInput_2.getDouble(5);
            if(rs_tDBInput_2.wasNull()){
                    throw new RuntimeException("Null value in non-Nullable column");
            }
		                    }
        	...
        	row3.LB_NOM_FICHIER = routines.system.JDBCUtil.getString(rs_tDBInput_2, 6, false);
		                    }
							if(colQtyInRs_tDBInput_2 < 7) {
								row3.DT_INSERTION = null;
							} else {
										
			row3.DT_INSERTION = routines.system.JDBCUtil.getDate(rs_tDBInput_2, 7);
		                    }
							if(colQtyInRs_tDBInput_2 < 8) {
								row3.LB_JOB_NAME = null;
							} else {
	                         		
        	row3.LB_JOB_NAME = routines.system.JDBCUtil.getString(rs_tDBInput_2, 8, false);
		                    }
 ...
 */
	...
	currentComponent="tAdvancedHash_row3";
				...
				row3_HashRow.LOCATION_ID = row3.LOCATION_ID;
				
				row3_HashRow.CITY = row3.CITY;
				
				row3_HashRow.ZIP_CODE = row3.ZIP_CODE;
 ...
 */
    ...
    public static void main(String[] args){
        final jLoadCustomerDim jLoadCustomerDimClass = new jLoadCustomerDim();

        int exitCode = jLoadCustomerDimClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = jLoadCustomerDim.class.getClassLoader().getResourceAsStream("churn_project/jloadcustomerdim_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = jLoadCustomerDim.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : jLoadCustomerDim");
        }
...
/************************************************************************************************
 *     228244 characters generated by Talend Open Studio for Data Integration 
