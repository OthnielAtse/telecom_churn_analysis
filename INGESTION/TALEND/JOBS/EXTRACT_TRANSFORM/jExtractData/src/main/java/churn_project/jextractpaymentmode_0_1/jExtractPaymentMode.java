...
package churn_project.jextractpaymentmode_0_1;
...
/**
 * Job: jExtractPaymentMode Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class jExtractPaymentMode implements TalendJob {
...
public String getProjectFolder(){
	...
	private final String jobName = "jExtractPaymentMode";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				jExtractPaymentMode.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(jExtractPaymentMode.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
java.io.File file_tFileExist_1 = new java.io.File(context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" + "CHURN_"+ TalendDate.getDate("YYYYMMDD") +"_PAYMENT_MODE.csv");
...
globalMap.put("tFileExist_1_FILENAME",context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" + "CHURN_"+ TalendDate.getDate("YYYYMMDD") +"_PAYMENT_MODE.csv");
...
public static class uniqueChurnStruct implements routines.system.IPersistableRow<uniqueChurnStruct> {
    final static byte[] commonByteArrayLock_CHURN_PROJECT_jExtractPaymentMode = new byte[0];
    static byte[] commonByteArray_CHURN_PROJECT_jExtractPaymentMode = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_CHURN_PROJECT_jExtractPaymentMode.length) {
				if(length < 1024 && commonByteArray_CHURN_PROJECT_jExtractPaymentMode.length == 0) {
   					commonByteArray_CHURN_PROJECT_jExtractPaymentMode = new byte[1024];
				} else {
   					commonByteArray_CHURN_PROJECT_jExtractPaymentMode = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_CHURN_PROJECT_jExtractPaymentMode, 0, length);
			strReturn = new String(commonByteArray_CHURN_PROJECT_jExtractPaymentMode, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_CHURN_PROJECT_jExtractPaymentMode, 0, length);
			strReturn = new String(commonByteArray_CHURN_PROJECT_jExtractPaymentMode, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_CHURN_PROJECT_jExtractPaymentMode) {
...
String fileName_tFileOutputDelimited_1 = "";
    fileName_tFileOutputDelimited_1 = (new java.io.File(context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" + "CHURN_"+ TalendDate.getDate("YYYYMMDD") +"_PAYMENT_MODE.csv")).getAbsolutePath().replace("\\","/");
...
customers_tmp.PaymentModeID = null;
customers_tmp.PaymentMethod = StringHandling.TRIM(row1.PaymentMethod) ;
...
row2 = null;			
if(customers.PaymentMethod == null){
	finder_tUniqRow_1.PaymentMethod = null;
}else{
	finder_tUniqRow_1.PaymentMethod = customers.PaymentMethod.toLowerCase();
}	
...
if(customers.PaymentMethod == null){
	new_tUniqRow_1.PaymentMethod = null;
}else{
	new_tUniqRow_1.PaymentMethod = customers.PaymentMethod.toLowerCase();
}
		
		keystUniqRow_1.add(new_tUniqRow_1);if(row2 == null){ 
	
	row2 = new row2Struct();
}row2.PaymentModeID = customers.PaymentModeID;			row2.PaymentMethod = customers.PaymentMethod;					
...
uniqueChurn_tmp.PaymentModeID = Var.id ;
uniqueChurn_tmp.PaymentMethod = row2.PaymentMethod ;
...
if(uniqueChurn != null) { 
	...
	currentComponent="tFileOutputDelimited_1";
                    ...
                    StringBuilder sb_tFileOutputDelimited_1 = new StringBuilder();
                            if(uniqueChurn.PaymentModeID != null) {
                        sb_tFileOutputDelimited_1.append(
                            uniqueChurn.PaymentModeID
                        );
                            }
                            sb_tFileOutputDelimited_1.append(OUT_DELIM_tFileOutputDelimited_1);
                            if(uniqueChurn.PaymentMethod != null) {
                        sb_tFileOutputDelimited_1.append(
                            uniqueChurn.PaymentMethod
                        );
                            }
 ...
 */
	...
	System.err.println("Le fichier " +"CHURN_"+ TalendDate.getDate("YYYYMMDD") +" _PAYMENT_MODE.csv" +"  existe déjà dans le repertoire du jour " + context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" );
	
	globalMap.put("tDie_1_DIE_MESSAGE", "Le fichier " +"CHURN_"+ TalendDate.getDate("YYYYMMDD") +" _PAYMENT_MODE.csv" +"  existe déjà dans le repertoire du jour " + context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" );
	globalMap.put("tDie_1_DIE_MESSAGES", "Le fichier " +"CHURN_"+ TalendDate.getDate("YYYYMMDD") +" _PAYMENT_MODE.csv" +"  existe déjà dans le repertoire du jour " + context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" );
 ...
 */
    ...
    public static void main(String[] args){
        final jExtractPaymentMode jExtractPaymentModeClass = new jExtractPaymentMode();

        int exitCode = jExtractPaymentModeClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = jExtractPaymentMode.class.getClassLoader().getResourceAsStream("churn_project/jextractpaymentmode_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = jExtractPaymentMode.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : jExtractPaymentMode");
        }
...
/************************************************************************************************
 *     153944 characters generated by Talend Open Studio for Data Integration 
