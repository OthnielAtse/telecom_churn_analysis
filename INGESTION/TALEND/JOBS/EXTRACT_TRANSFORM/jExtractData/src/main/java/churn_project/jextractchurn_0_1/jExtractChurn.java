...
package churn_project.jextractchurn_0_1;
...
/**
 * Job: jExtractChurn Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class jExtractChurn implements TalendJob {
...
public String getProjectFolder(){
	...
	private final String jobName = "jExtractChurn";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				jExtractChurn.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(jExtractChurn.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
java.io.File file_tFileExist_1 = new java.io.File(context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" + "CHURN_"+ TalendDate.getDate("YYYYMMDD") +"_CHURN.csv");
...
globalMap.put("tFileExist_1_FILENAME",context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" + "CHURN_"+ TalendDate.getDate("YYYYMMDD") +"_CHURN.csv");
...
public static class uniqueChurnStruct implements routines.system.IPersistableRow<uniqueChurnStruct> {
    final static byte[] commonByteArrayLock_CHURN_PROJECT_jExtractChurn = new byte[0];
    static byte[] commonByteArray_CHURN_PROJECT_jExtractChurn = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_CHURN_PROJECT_jExtractChurn.length) {
				if(length < 1024 && commonByteArray_CHURN_PROJECT_jExtractChurn.length == 0) {
   					commonByteArray_CHURN_PROJECT_jExtractChurn = new byte[1024];
				} else {
   					commonByteArray_CHURN_PROJECT_jExtractChurn = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_CHURN_PROJECT_jExtractChurn, 0, length);
			strReturn = new String(commonByteArray_CHURN_PROJECT_jExtractChurn, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_CHURN_PROJECT_jExtractChurn, 0, length);
			strReturn = new String(commonByteArray_CHURN_PROJECT_jExtractChurn, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_CHURN_PROJECT_jExtractChurn) {
     ...
     * Compare keys
     */
    public int compareTo(uniqueChurnStruct other) {
...
uniqueChurnStruct uniqueChurn = new uniqueChurnStruct();
	...
	currentComponent="tFileOutputDelimited_1";
					...
					if(execStat) {
						runStat.updateStatOnConnection(resourceMap,iterateId,0,0,"uniqueChurn");
					}
...
String fileName_tFileOutputDelimited_1 = "";
    fileName_tFileOutputDelimited_1 = (new java.io.File(context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" + "CHURN_"+ TalendDate.getDate("YYYYMMDD") +"_CHURN.csv")).getAbsolutePath().replace("\\","/");
...
uniqueChurnStruct uniqueChurn_tmp = new uniqueChurnStruct();
// ###############################
...
customers_tmp.ChurnId = null;
customers_tmp.CustomerStatus = StringHandling.TRIM(row1.CustomerStatus) ;
customers_tmp.ChurnCategory = StringHandling.TRIM(row1.ChurnCategory) ;
customers_tmp.ChurnReason = StringHandling.TRIM(row1.ChurnReason) ;
...
if(customers.CustomerStatus == null){
	finder_tUniqRow_1.CustomerStatus = null;
}else{
	finder_tUniqRow_1.CustomerStatus = customers.CustomerStatus.toLowerCase();
}
if(customers.ChurnCategory == null){
	finder_tUniqRow_1.ChurnCategory = null;
}else{
	finder_tUniqRow_1.ChurnCategory = customers.ChurnCategory.toLowerCase();
}
if(customers.ChurnReason == null){
	finder_tUniqRow_1.ChurnReason = null;
}else{
	finder_tUniqRow_1.ChurnReason = customers.ChurnReason.toLowerCase();
}	
...
if(customers.CustomerStatus == null){
	new_tUniqRow_1.CustomerStatus = null;
}else{
	new_tUniqRow_1.CustomerStatus = customers.CustomerStatus.toLowerCase();
}
if(customers.ChurnCategory == null){
	new_tUniqRow_1.ChurnCategory = null;
}else{
	new_tUniqRow_1.ChurnCategory = customers.ChurnCategory.toLowerCase();
}
if(customers.ChurnReason == null){
	new_tUniqRow_1.ChurnReason = null;
}else{
	new_tUniqRow_1.ChurnReason = customers.ChurnReason.toLowerCase();
}
		
		keystUniqRow_1.add(new_tUniqRow_1);if(row2 == null){ 
	
	row2 = new row2Struct();
}row2.ChurnId = customers.ChurnId;			row2.CustomerStatus = customers.CustomerStatus;			row2.ChurnCategory = customers.ChurnCategory;			row2.ChurnReason = customers.ChurnReason;					
...
// # Output table : 'uniqueChurn'
uniqueChurn_tmp.ChurnId = Var.id ;
uniqueChurn_tmp.CustomerStatus = row2.CustomerStatus ;
uniqueChurn_tmp.ChurnCategory = row2.ChurnCategory ;
uniqueChurn_tmp.ChurnReason = row2.ChurnReason ;
uniqueChurn = uniqueChurn_tmp;
// ###############################
...
if(uniqueChurn != null) { 
	...
	currentComponent="tFileOutputDelimited_1";
                    ...
                    StringBuilder sb_tFileOutputDelimited_1 = new StringBuilder();
                            if(uniqueChurn.ChurnId != null) {
                        sb_tFileOutputDelimited_1.append(
                            uniqueChurn.ChurnId
                        );
                            }
                            sb_tFileOutputDelimited_1.append(OUT_DELIM_tFileOutputDelimited_1);
                            if(uniqueChurn.CustomerStatus != null) {
                        sb_tFileOutputDelimited_1.append(
                            uniqueChurn.CustomerStatus
                        );
                            }
                            sb_tFileOutputDelimited_1.append(OUT_DELIM_tFileOutputDelimited_1);
                            if(uniqueChurn.ChurnCategory != null) {
                        sb_tFileOutputDelimited_1.append(
                            uniqueChurn.ChurnCategory
                        );
                            }
                            sb_tFileOutputDelimited_1.append(OUT_DELIM_tFileOutputDelimited_1);
                            if(uniqueChurn.ChurnReason != null) {
                        sb_tFileOutputDelimited_1.append(
                            uniqueChurn.ChurnReason
                        );
                            }
 ...
 */
	...
	currentComponent="tFileOutputDelimited_1";
		...
		resourceMap.put("finish_tFileOutputDelimited_1", true);
				...
				if(execStat){
			  		runStat.updateStat(resourceMap,iterateId,2,0,"uniqueChurn");
			  	}
 ...
 */
	...
	System.err.println("Le fichier " +"CHURN_"+ TalendDate.getDate("YYYYMMDD") +" _CHURN.csv" +"  existe déjà dans le repertoire du jour " + context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" );
	
	globalMap.put("tDie_1_DIE_MESSAGE", "Le fichier " +"CHURN_"+ TalendDate.getDate("YYYYMMDD") +" _CHURN.csv" +"  existe déjà dans le repertoire du jour " + context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" );
	globalMap.put("tDie_1_DIE_MESSAGES", "Le fichier " +"CHURN_"+ TalendDate.getDate("YYYYMMDD") +" _CHURN.csv" +"  existe déjà dans le repertoire du jour " + context.dataFolder + TalendDate.getDate("YYYYMMDD") + "/" );
 ...
 */
    ...
    public static void main(String[] args){
        final jExtractChurn jExtractChurnClass = new jExtractChurn();

        int exitCode = jExtractChurnClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = jExtractChurn.class.getClassLoader().getResourceAsStream("churn_project/jextractchurn_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = jExtractChurn.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : jExtractChurn");
        }
...
/************************************************************************************************
 *     158551 characters generated by Talend Open Studio for Data Integration 
 *     on the 31 décembre 2025 à 11:36:17 EET
 ************************************************************************************************/